      ...
      <div className="relative z-10 max-w-sm mx-auto space-y-5">
        ...
        >
          <div className="relative">
            ...
            >
              {photoUrl ? (
                <img src={photoUrl} alt={petName || "Pet"} className="w-full h-full object-cover" />
              ) : (
          ...
          <div className="text-center mt-4 space-y-1">
            ...
            {ownerName && <p className="text-sm text-white/60">Tutor: {ownerName}</p>}
          </div>
        </motion.div>
              ...
              {galleryPhotos.length > 1 && (
                <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                  {galleryPhotos.map((_, index) => (
                    <button
                      key={index}
                      className={`w-2 h-2 rounded-full transition-all ${index === currentSlide ? "w-4" : "opacity-50"}`}
        ...
        {/* Footer */}
        <p className="text-center text-xs text-white/40 pt-4">Tag Tá Na Mão 🏷️</p>
      </div>
    </div>
  );
};
