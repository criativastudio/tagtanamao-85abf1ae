...
export default function Auth() {
  ...
  const [errors, setErrors] = useState<{ email?: string; password?: string; confirmPassword?: string; activationCode?: string }>({});
  ...
  const handleValidateActivationCode = async () => {
    if (!activationCode.trim() || !productType) {
      setErrors({ activationCode: 'Digite o código de ativação' });
      return;
    }
    ...
    try {
      const { data, error } = await supabase.functions.invoke('validate-activation-code', {
        body: { code: activationCode.trim(), type: productType }
      });
    ...
    >
      ...
      <div className="space-y-2">
        ...
        {errors.activationCode && (
          <p className="text-sm text-destructive">{errors.activationCode}</p>
        )}
      </div>
  ...
  const renderAccountForm = () => (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
    >
      {!isLogin && validatedProduct && (
        <div className="mb-6 p-3 rounded-lg bg-primary/10 border border-primary/20">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
              validatedProduct.productType === 'pet_tag' ? 'bg-primary/20' : 'bg-blue-500/20'
            }`}>
              {validatedProduct.productType === 'pet_tag' 
