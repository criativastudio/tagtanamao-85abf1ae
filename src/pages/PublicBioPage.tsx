  ...
  useEffect(() => {
    const fetchBioPage = async () => {
      ...
      try {
        ...
        const galleryPhotos = Array.isArray(data.gallery_photos) ? (data.gallery_photos as unknown as string[]) : [];
        const buttons = Array.isArray(data.buttons) ? (data.buttons as unknown as BioButton[]) : [];
  ...
  if (loading) {
    return (
      <div
        ...
        style={{ backgroundColor: `hsl(${DEFAULT_THEME.backgroundColor})` }}
      >
